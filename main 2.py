...
class Main(QMainWindow):
    ...
    def function(self):
        ...
        try:
            ...
            if self.min_x < 0 or self.max_x < 0 or self.min_y < 0 or self.max_y < 0:
                self.err()
                self.ui.error_data(error_name = "Минимум по X и/или Y не может быть отрицательным!")
            else:
                if self.min_x > self.max_x:
                ...
                for x in x_list:
                    ...
                    if (fun != ""):
                        try:
                            ans = eval(fun.replace("^", "**").replace("x", f"{x}").replace("X", f"{x}"))
                            ...
                            if len(x_list_final) <= 0:
                                self.ui.error_data(error_code = "Нет доступных значений")
                            else:
